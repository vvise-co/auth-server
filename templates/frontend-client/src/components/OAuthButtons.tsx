...
/**
 * OAuth provider buttons that redirect to the central auth server.
 */
export default function OAuthButtons({ callbackUrl = '/dashboard' }: OAuthButtonsProps) {
  const handleLogin = (provider: string) => {
    // Construct the callback URL for this app
    const appCallbackUrl = `${window.location.origin}/auth/callback?callbackUrl=${encodeURIComponent(callbackUrl)}`;

    // Redirect to auth server's OAuth endpoint
    window.location.href = `${AUTH_SERVER_URL}/oauth2/authorization/${provider}?redirect_uri=${encodeURIComponent(appCallbackUrl)}`;
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        ...
        className="w-full flex items-center justify-center px-4 py-3 border border-gray-300 rounded-md shadow-sm bg-gray-900 text-white hover:bg-gray-800 transition-colors"
      >
