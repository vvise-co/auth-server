...
export default function OAuthButtons({ redirectUri }: OAuthButtonsProps) {
  const { login } = useAuth();

  const handleLogin = (provider: 'google' | 'github' | 'microsoft') => {
    if (redirectUri) {
      // Store redirect URI in cookie for OAuth flow
      document.cookie = `oauth2_redirect_uri=${encodeURIComponent(redirectUri)}; path=/; max-age=600`;
    }
    login(provider);
  };
